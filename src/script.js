...
const inputelem = document.querySelector('#inputtab');
var allrepostorage;
var val;
var totalrepo;
var gpage=0;
...
document.getElementById('inputid').onkeydown = function(e){
 ...
 async function changeHandler(){
    let inpVal = inputelem.value;
    const filteredResponse = await allrepostorage.filter(item => item.name.startsWith(inpVal));
    const component = filteredResponse.map((item,index)=>{
      return `
